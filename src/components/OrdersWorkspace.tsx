...
export function OrdersWorkspace({ orders, budget, delivered, canCreate, canTakePart, openDialog, initialFilter, canMove, onMove, onReturn, onReprint }: {
  ...
  */
  ...
  const abrirEntregues = () => { setView("cards"); setFilter("Finalizadas (entregues)"); setLimit(30); };
  ...
  const cartao = (order: OrderRecord) => <article className={`work-order-card ${orderIsLate(order) ? "is-late" : ""}`} key={order.id} onDoubleClick={() => openDialog("order", order.id)} title="Dois cliques abrem a OS">
    <header><span className="order-number">{order.id}{order.partnerOrderId ? <em className="partner-order"> · {order.partnerName || "parceiro"} {order.partnerOrderId}</em> : null}</span><span className={`status ${order.closed ? "neutral" : statusTone(order.status)}`}><i/>{order.closed ? "Entregue" : normalizeOrderStatus(order.status)}</span></header>
    <h2>{order.customer || "Cliente a identificar"}</h2>
    {order.returnOfOrderId ? <p className="return-flag"><Icon name="alert" size={14}/>Retorno da {order.returnOfOrderId}</p> : null}
    ...
    <footer>
      <span>{orderAttention(order) || (order.total != null ? money(order.total) : "Atendimento em aberto")}</span>
      {/* "Retorno" só na moto que já saiu da oficina: é para quando a
          concessionária traz de volta a mesma moto pelo mesmo problema. Um
          clique abre uma OS nova com o cadastro já preenchido — sem redigitar
          cliente, moto e parceira, e sem trazer a conta que já foi paga. */}
      {order.closed && onReturn ? <button className="outline-button return-button" onClick={() => void onReturn(order)} onDoubleClick={(event) => event.stopPropagation()} title={`Abrir uma OS nova como retorno da ${order.id}`}><Icon name="arrow" size={15}/>Retorno</button> : null}
      <button className="outline-button" onClick={() => openDialog("order", order.id)} onDoubleClick={(event) => event.stopPropagation()}>Abrir OS <Icon name="arrow" size={16}/></button>
    </footer>
  </article>;
